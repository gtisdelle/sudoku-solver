...
class SudokuController(object):
    ...
    def new_game(self):
        """Reset the puzzle to a new game."""
        self.model.setup()

    def reset(self):
        """Reset the puzzle to its original state (before any user inputs)."""
    ...
    def cell_clicked(self, view, event):
        ...
        Sets the row and col attributes of the view to the currently
        selected rows and columns.

        Args:
            view: the view
            event: the Event generated by the mouse click
        """
        x_coord = event.x
        y_coord = event.y

        if (MARGIN < x_coord < WIDTH - MARGIN and MARGIN < y_coord < HEIGHT - MARGIN):
            event.widget.focus_set()

            row = int((y_coord - MARGIN) / SIDE)
            col = int((x_coord - MARGIN) / SIDE)
    ...
    def number_pressed(self, view, event):
        """Set the selected cell to the user input.

        Args:
            view: the view
            event: the Event generated by the key press
        """
...
class SudokuView(Frame):
    ...
    def draw_cursor(self):
        ...
        if self._row >= 0 and self._col >= 0:
            x_0 = MARGIN + self._col * SIDE + 1
            y_0 = MARGIN + self._row * SIDE + 1
            x_1 = MARGIN + (self._col + 1) * SIDE - 1
            y_1 = MARGIN + (self._row + 1) * SIDE - 1
            self.canvas.create_rectangle(
                x_0, y_0, x_1, y_1, outline="red", tags="cursor")

    def handle_notification(self, subject):
    ...
    def _draw_grid(self):
        ...
        for i in range(10):
            ...
            x_1 = MARGIN + i * SIDE
            y_1 = HEIGHT - MARGIN
            self.canvas.create_line(x_0, y_0, x_1, y_1, fill=color)
            ...
            for j in range(9):
                ...
                if answer != 0:
                    ...
                    if answer == generated:
                        ...
                        color = "sea green"
                    self.canvas.create_text(x_coord, y_coord, text=answer, tags="numbers",
                                            fill=color)
